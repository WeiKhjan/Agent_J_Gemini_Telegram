...
bot.command('test', (ctx) => {
  try {
    ...
    const ver = execSync('gemini --version', { encoding: 'utf8', shell: true, timeout: 10000 });
    ctx.reply(`Gemini CLI: ✅ ${ver.trim()}`);
  } catch (e) {
    ctx.reply(`Gemini CLI: ❌ ${e.message}`);
  }
});
...
bot.on('text', async (ctx) => {
  ...
  setImmediate(async () => {
    try {
      ...
      const result = await runGemini(ctx.message.text, sessionId);
...
const handleMedia = async (ctx, getFile, prompt) => {
  ...
  try {
    ...
    await sendResponse(ctx.telegram, ctx.chat.id, result.response);
  } catch (e) {
    await ctx.telegram.deleteMessage(ctx.chat.id, msg.message_id).catch(() => {});
...
(async () => {
  ...
  if (webhookUrl && !webhookUrl.includes('your-subdomain')) {
    ...
    app.get('/', (_, res) => res.send('Tele Agent J (Gemini) running!'));
